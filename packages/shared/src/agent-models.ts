...
export interface AgentModelOption {
	id: string;
	label: string;
	/** Optional runtime grouping label, used by multi-provider agents. */
	provider?: string;
}
...
export const AGENT_MODEL_SUPPORT: readonly AgentModelSupport[] = [
	...
	},
	{
		...
		modelFlag: "--model",
		// Antigravity reports the subscription-specific catalog through
		// `agy models`; there is no safe account-independent fallback.
		models: [],
	},
	{
		presetId: "copilot",
	...
	},
	{
		presetId: "pi",
		modelFlag: "--model",
		// Pi's provider-aware inventory is discovered from its RPC protocol.
		// Keep no static fallback because availability depends on user credentials.
		models: [],
	},
	{
		presetId: "grok",
		modelFlag: "--model",
		// Grok reports the exact account catalog through `grok models`.
		models: [],
	},
	{
		presetId: "kimi",
		modelFlag: "--model",
		// Kimi exposes user-configured aliases through `provider list --json`.
		models: [],
	},
	{
		presetId: "vibe",
...
export interface AgentEffortSupport {
	...
	label?: string;
	effortFlag?: string;
	/** Agent/model default. Selecting it omits the override flag. */
 ...
 * (gemini, cursor-agent, droid, superset chat) expose no effort control on
 * their interactive launch command. OpenCode's runtime narrows its generic
 * variant transport to the exact levels advertised by each model.
 */
...
/**
 * Launch-time performance modes that are independent from model and
 * reasoning effort. Standard explicitly disables the agent's fast mode so
 * this picker overrides a user's global default.
 */
...
/** Agent personas/modes that are independent from model reasoning. */
...
function normalizeAgentEffort(
	...
	if (presetId === "claude" && effort === "ultracode") {
		return "xhigh";
	}
	if (
		presetId === "claude" &&
...
export function buildAgentEffortArgs(
	...
	if (!effort) return [];
	const staticSupport = AGENT_EFFORT_SUPPORT.find(
		(entry) => entry.presetId === presetId,
	);
	const support = runtimeProfile
		? staticSupport && { ...staticSupport, ...runtimeProfile }
		: getAgentEffortSupport(presetId, model);
	if (!support) return [];
	if (!support.efforts.some((option) => option.id === effort)) return [];
	if (
		!support.effortFlag ||
		(presetId === "claude" && model === "claude-haiku-4-5")
	) {
		return [];
	}
...
/**
 * Runtime flags shared by launch traits. Claude settings must be emitted as a
 * single JSON object because repeated `--settings` flags do not compose
 * reliably. Other agents can use their independent speed flags directly.
 */
